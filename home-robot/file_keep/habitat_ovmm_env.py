...
class HabitatOpenVocabManipEnv(HabitatEnv):
    ...
    def _preprocess_obs(
        ...
        start_gps,rotation_world_start,origin = habitat_obs["robot_start_gps"]
        obs = home_robot.core.interfaces.Observations(
            rgb=habitat_obs["robot_head_rgb"],
            depth=depth,
            compass=habitat_obs["robot_start_compass"],
            gps=self._preprocess_xy(start_gps),
            task_observations={
                ...
                "robot_start_gps":habitat_obs["robot_start_gps"],
                "robot_start_compass":habitat_obs["robot_start_compass"],
            },
        ...
        obs.rotation_world_start = rotation_world_start
        obs.origin = origin
    ...
    def _preprocess_semantic(
        ...
        recep_seg,episode,now_sim,task = habitat_obs["receptacle_segmentation"]
        obs.now_sim = now_sim
        obs.episode = episode
        obs.task = task
        if self.ground_truth_semantics:
            ...
            recep_seg = (
                recep_seg.squeeze(-1).astype(np.int64)
            )
    ...
    def _preprocess_action(
        ...
        if type(action) in [ContinuousFullBodyAction, ContinuousNavigationAction]:
            ...
            elif action == DiscreteNavigationAction.MOVE_FORWARD_SMALL:
                forward = self.discrete_forward/2
