...
class ResNetEncoder2(nn.Module):
    ...
    def forward(self, cnn_input) -> torch.Tensor:  # type: ignore
        if self.is_blind:
            return None
...
class ResNetEncoder3(nn.Module):
    def __init__(
        self,
        baseplanes: int = 32,
        ngroups: int = 32,
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        print("observation_space.spaces.keys():",observation_space.spaces.keys())
        # visual encoder
        ...
        self._fuse_keys_1d = ['ovmm_object_position','robot_start_gps', 'robot_start_compass']
        # self._fuse_keys_1d = ['object_embedding','robot_start_gps', 'robot_start_compass']
        if len(self._fuse_keys_1d) != 0:
            # fuse_keys_1d_dim = sum(
            #     observation_space.spaces[k].shape[0]
            #     for k in self._fuse_keys_1d
            # )
            fuse_keys_1d_dim = 5
            self.fuse_keys_1d_embedding = nn.Linear(fuse_keys_1d_dim, 64)
            rnn_input_size += 64
            
            # rnn_input_size += fuse_keys_1d_dim
        ...
        fuse_keys = ['robot_head_depth']
        # fuse_keys = ['robot_head_depth','receptacle_segmentation','ovmm_nav_goal_segmentation']

        if force_blind_policy:  # no
        ...
        self.visual_encoder2 = ResNetEncoder2(
            baseplanes=resnet_baseplanes,
            ngroups=resnet_baseplanes // 2,
            make_backbone=getattr(resnet_gn if ovrl else resnet, backbone),
            no_downscaling=no_downscaling,
        ...
        if not self.visual_encoder.is_blind:    # yes
            ...
            self.visual_fc2 = nn.Sequential(
                nn.Flatten(),
                nn.Linear(
                    np.prod(self.visual_encoder2.output_shape), hidden_size
                ),
                nn.ReLU(True),
            )
            rnn_input_size += self._hidden_size
        # print("rnn_input_size:",rnn_input_size)   # 579
        # print("self.is_blind:",self.is_blind) # False
        # print("self._hidden_size:",self._hidden_size) # 512
    ...
    def forward(
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                ...
                # visual_feats = self.visual_encoder(observations['local_map'])    # 
                # visual_feats = self.visual_encoder(torch.from_numpy(observations['local_map']).to(rnn_hidden_states.device))    # 

            visual_feats = self.visual_fc(visual_feats)
            aux_loss_state["perception_embed"] = visual_feats
            x.append(visual_feats)
            
            visual_feats2 = self.visual_encoder2(observations['local_map'])    # 

            visual_feats2 = self.visual_fc2(visual_feats2)
            x.append(visual_feats2)
        if len(self._fuse_keys_1d) != 0:
            fuse_states = torch.cat(
                [observations[k] for k in self._fuse_keys_1d], dim=-1
            )
            fuse_states_feat = self.fuse_keys_1d_embedding(fuse_states.float())
            x.append(fuse_states_feat)
            
            # x.append(fuse_states.float())

        if IntegratedPointGoalGPSAndCompassSensor.cls_uuid in observations: # no
        ...
        if PointGoalSensor.cls_uuid in observations:    # no
            goal_observations = observations[PointGoalSensor.cls_uuid]
            x.append(self.pointgoal_embedding(goal_observations))

        if ProximitySensor.cls_uuid in observations:    # no
            sensor_observations = observations[ProximitySensor.cls_uuid]
            x.append(self.proximity_embedding(sensor_observations))

        if HeadingSensor.cls_uuid in observations:  # no
        ...
        if ObjectGoalSensor.cls_uuid in observations:   # no
            object_goal = observations[ObjectGoalSensor.cls_uuid].long()
            x.append(self.obj_categories_embedding(object_goal).squeeze(dim=1))

        if ObjectCategorySensor.cls_uuid in observations:   # no
        ...
        if StartReceptacleSensor.cls_uuid in observations:  # yes
        ...
        if GoalReceptacleSensor.cls_uuid in observations:   # no
        ...
        if EpisodicCompassSensor.cls_uuid in observations:  # no
        ...
        if EpisodicGPSSensor.cls_uuid in observations:  # no
            x.append(
...
class PointNavResNetNet2(Net):
    ...
    def __init__(
        ...
        self._fuse_keys_1d = ['robot_start_gps', 'robot_start_compass']
        if len(self._fuse_keys_1d) != 0:
            fuse_keys_1d_dim = sum(
                observation_space.spaces[k].shape[0]
                for k in self._fuse_keys_1d
        ...
        fuse_keys = ['robot_head_depth','ovmm_nav_goal_segmentation','receptacle_segmentation']
...
# point nav with map (map goal)
class PointNavResNetNet3(Net): 
    """Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
...
class PointNavResNetNet_ori(Net): 
    """Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
    ...
    def __init__(
        ...
        self._fuse_keys_1d = ['joint', 'object_embedding','robot_start_compass', 'robot_start_gps']
        # self._fuse_keys_1d = ['object_embedding','robot_start_gps', 'robot_start_compass']
        if len(self._fuse_keys_1d) != 0:
            fuse_keys_1d_dim = sum(
                observation_space.spaces[k].shape[0]
        ...
        # fuse_keys = ['robot_head_depth']
        fuse_keys = ['joint', 'object_embedding', 'ovmm_nav_goal_segmentation','receptacle_segmentation','robot_head_depth','robot_start_compass', 'robot_start_gps']
...
class PointNavResNetNet_ori_re(Net):
    """Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
    ...
    def __init__(
        ...
        # self._fuse_keys_1d = ['joint', 'object_embedding','robot_start_compass', 'robot_start_gps']
        self._fuse_keys_1d = ['object_embedding','robot_start_compass', 'robot_start_gps']
        if len(self._fuse_keys_1d) != 0:
            rnn_input_size += sum(
                observation_space.spaces[k].shape[0]
                for k in self._fuse_keys_1d
        ...
        fuse_keys = ['object_embedding', 'ovmm_nav_goal_segmentation','receptacle_segmentation','robot_head_depth','robot_start_compass', 'robot_start_gps']
        if force_blind_policy:  # no
            use_obs_space = spaces.Dict({})
        else:
            use_obs_space = spaces.Dict(
...
class PointNavResNetNet_ori_re2(Net):
    """Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
    ...
    def __init__(
        ...
        if len(self._fuse_keys_1d) != 0:
            ...
            # self.fuse_keys_1d_embedding = nn.Linear(fuse_keys_1d_dim, 32)
            # rnn_input_size += 32
        ...
        # fuse_keys = ['robot_head_depth']
        fuse_keys = ['ovmm_nav_goal_segmentation','receptacle_segmentation','robot_head_depth']
        if force_blind_policy:  # no
            use_obs_space = spaces.Dict({})
        else:
            use_obs_space = spaces.Dict(
        ...
        self.visual_encoder2 = ResNetEncoder3(
            baseplanes=resnet_baseplanes,
            ngroups=resnet_baseplanes // 2,
            make_backbone=getattr(resnet_gn if ovrl else resnet, backbone),
            no_downscaling=no_downscaling,
...
# object nav with map (map goal)
class PointNavResNetNet6(Net): 
    """Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
