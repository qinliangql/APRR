...
class DiscretePlanner:
    ...
    def plan(
        ...
        # print("goal_map:",np.sum(goal_map))
    ...
    def _get_short_term_goal(
        ...
        if self.last_action is not None:
            # repetitions = self.detect_local_stuck(self.last_action)
            repetitions = self.detect_local_stuck(self.curr_pose)
            if len(repetitions)>0 and frontier_map is not None:
                astar_planner = AStarPlanner()
                # goal_map_dia = (fast_binary_dilation(goal_map, skimage.morphology.disk(5)) == True)*1.0
                # astar_flag, stg_y, stg_x, closest_goal_pt, path = astar_planner.get_shortterm_goal(goal_map_dia,state,traversible)
                # if path is not None and len(path)==0:   # unreachable pos
