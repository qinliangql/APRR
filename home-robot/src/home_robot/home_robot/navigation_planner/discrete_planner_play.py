...
def fast_binary_dilation(grid, selem):
    selem = np.array(selem, dtype=np.uint8)
    grid = grid.astype(np.uint8)
    dilated = cv2.dilate(grid, selem)
...
class DiscretePlanner:
    ...
    def __init__(
        ...
        self.default_local_map_dir = f"/aiarena/gpfs/code/code/OVMM/home-robot/train_data/local_map_pos2_dg/{exp_name}"
        ...
        self.stet_list_len = 60
    
    def detect_local_stuck(self,state, min_segment_length=10, min_repetitions=3):
        self.state_list.append(round(state[0]+state[1],2))
        # self.state_list.append(state.value)
        repetitions = []
        if len(self.state_list) > self.stet_list_len:
            self.state_list.popleft()
            n = self.stet_list_len
            trajectory = list(self.state_list)
            for i in range(n - min_segment_length * min_repetitions + 1):
                for length in range(min_segment_length, (n - i) // min_repetitions + 1):
                    segment = trajectory[i:i+length]
                    repeat_count = 1
                    j = i + length

                    while j + length <= n and trajectory[j:j+length] == segment:
                        repeat_count += 1
                        j += length

                    if repeat_count >= min_repetitions:
                        repetitions.append((segment, repeat_count, i, j))
                        break  # 找到最长的重复序列后跳出内层循环
    ...
    def set_local_map_dir(self, scene_id: str, episode_id: str):
        self.local_map_dir = os.path.join(self.default_local_map_dir, f"{scene_id}_{episode_id}")
        shutil.rmtree(self.local_map_dir, ignore_errors=True)
    ...
    def plan(
        ...
        goal_map_save = None
        # if len(relative_stg) > 0 and (dis_target < 5 or (-1 < np.sum(frontier_map -goal_map) < 1) or found_goal):
        if len(relative_stg) > 0:
            # set_goal = [
            #     int(start[0]+(relative_stg[1])*100.0/self.map_resolution),
            #     int(start[1]+(relative_stg[0])*100.0/self.map_resolution),
            # ]
            set_goal = [
                int((start_y + relative_stg[1])*100.0/self.map_resolution - gx1),
                int((start_x + relative_stg[0])*100.0/self.map_resolution - gy1),
            ]
            set_goal = pu.threshold_poses(set_goal, goal_map.shape)
            goal_map = goal_map*0.0
            # for i in [-1,0,1]:
            #     for j in [-1,0,1]:
            #         if (0 <= (set_goal[1]+i) < goal_map.shape[0]) and (0 <= (set_goal[0]+j) < goal_map.shape[1]):
            goal_map[set_goal[0],set_goal[1]] = 1.0
            goal_map_save = goal_map*1.0
            
            # # 矩形的宽度和高度
            # rect_size = 4  # 这里你可以根据需要调整矩形的大小
            # half_rect_size = rect_size // 2

            # # 计算矩形左上角和右下角的坐标
            # top_left = (int(set_goal[0] - half_rect_size), int(set_goal[1] - half_rect_size))
            # bottom_right = (int(set_goal[0] + half_rect_size), int(set_goal[1] + half_rect_size))

            # # 使用 cv2.rectangle 在 goal_map 上绘制矩形
            # cv2.rectangle(goal_map, top_left, bottom_right, color=1, thickness=-1)  # 填充矩形
        
        if len(max_index) > 0:
            # for i in [-1,0,1]:
            #     for j in [-1,0,1]:
            #         if (0 <= (max_index[0]+i) < goal_map.shape[0]) and (0 <= (max_index[1]+j) < goal_map.shape[1]):
            goal_map[max_index[0],max_index[1]] = 1.0
        ...
        return action, closest_goal_map, short_term_goal, dilated_obstacles, goal_map_save
    ...
    def _get_short_term_goal(
        ...
        # Create inverse map of obstacles - this is territory we assume is traversible, 1 reachable 0 unreachable
        # Traversible is now the map
        traversible = 1 - dilated_obstacles
        traversible[self.collision_map[gx1:gx2, gy1:gy2][x1:x2, y1:y2] == 1] = 0
        # cv2.imshow("self.collision_map",np.flipud(self.collision_map[gx1:gx2, gy1:gy2][x1:x2, y1:y2]*255))
        # cv2.waitKey(1)
        ...
        state = [start[0] - x1 + 1, start[1] - y1 + 1]  # agent pos
        astar_flag = False
        
        if self.last_action is not None:
            if self.col_width > 7 and frontier_map is not None: # 帮助脱困
                astar_planner = AStarPlanner()
                goal_map_dia = add_boundary(frontier_map, value=0)
                astar_flag, stg_y, stg_x, closest_goal_pt, path = astar_planner.get_shortterm_goal(goal_map_dia,state,traversible)
                if astar_flag:
                    replan, stop, goal_distance_map = False, False, None
        ...
        if not astar_flag:
            ...
            if plan_to_dilated_goal:
                ...
                if len(np.unique(self.dd)) < 10000:
                    # print("replan with frontier")
                    # frontier_map = add_boundary(frontier_map, value=0)
                    # navigable_goal_map = frontier_map
                    # self.dd = planner.set_multi_goal(
                    #     navigable_goal_map,
            ...
            for i in range(length):
                for j in range(width):
                    ...
                    if self.col_width > 4:
                        for a in range(3):
                            for b in range(3):
                                x_tmp, y_tmp = r+a-1, c+b-1
                                if -1 < x_tmp < self.collision_map.shape[0] and -1 < y_tmp < self.collision_map.shape[1]:
                                    self.collision_map[x_tmp, y_tmp] = 1
