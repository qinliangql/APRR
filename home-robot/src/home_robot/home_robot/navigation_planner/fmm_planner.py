...
class FMMPlanner:
    ...
    def set_multi_goal(
        ...
        self.fmm_dist = dd
        # if timestep > 480 and flag:
        #     # np.save(f"/aiarena/gpfs/code/code/OVMM/home-robot/mytest/keep/traversible_ma_data_{timestep}.npy",traversible_ma.data)
        #     # np.save(f"/aiarena/gpfs/code/code/OVMM/home-robot/mytest/keep/traversible_ma_mask_{timestep}.npy",traversible_ma.mask)
        #     # np.save(f"/aiarena/gpfs/code/code/OVMM/home-robot/mytest/keep/self_fmm_dist_{timestep}.npy",self.fmm_dist)
        #     # np.save(f"/aiarena/gpfs/code/code/OVMM/home-robot/mytest/keep/goal_map_{timestep}.npy",goal_map)
        #     plt.subplot(121)
        #     plt.imshow(np.flipud(traversible_ma))
        #     plt.subplot(122)
        #     plt.imshow(np.flipud(self.fmm_dist))
        #     plt.show()
    ...
    def get_short_term_goal(
        ...
        """
        scale = self.scale * 1.0    # 1
        state = [x / scale for x in state]
        dx, dy = state[0] - int(state[0]), state[1] - int(state[1]) # 0,0
        ...
        subset2 = dist[
            state[0] -4 * self.du : state[0] + 6 * self.du + 1, state[1] -4 * self.du : state[1] + 6 * self.du + 1
        ]
        ...
        self.goal_tolerance = 0.005 # debug play
        stop = subset[self.du, self.du] < self.goal_tolerance   # agent位置距离目标的距离

        subset -= subset[self.du, self.du]
        ratio1 = subset / dist_mask     # dist_mask 中间八角形区域为5 周围值很小
        subset[ratio1 < -1.5] = 1   # 理论上比中心点更接近目标的点最多也就为-5，比率也就为-1，比这个更小不合理
    ...
    #     goal = cv2.dilate(
    #         goal, skimage.morphology.disk(3), iterations=1
    #     )
    ...
    #     cv2.imshow("planner.fmm_dist",(np.flipud(planner.fmm_dist)*255).astype(np.uint8))
    #     # navigable_goal_map = planner.fmm_dist < distance
    #     if len(np.unique(planner.fmm_dist)) ==2:    # can not reach object
    #         navigable_goal_map = np.zeros_like(goal)
    #     else:
    #         navigable_goal_map = goal
    #     cv2.imshow("navigable_goal_map",(np.flipud(navigable_goal_map)*255).astype(np.uint8))
    #     cv2.waitKey(1)
