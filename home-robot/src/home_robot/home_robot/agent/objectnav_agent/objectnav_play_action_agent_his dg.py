...
from home_robot.navigation_planner.action_plan_net import NavigatorWithAction
...
class ObjectNavAgent(Agent):
    ...
    def __init__(
        ...
        self.config = config
        self.arrive_view = False
        
        self.action_net = NavigatorWithAction()
        self.action_net.to(self.device)
        action_net_filepath = "/aiarena/gpfs/code/code/OVMM/Network/all_action5dd_checkpoints/best_checkpoint.pth.tar"
        self.action_net.load_state_dict(torch.load(action_net_filepath)['model_state_dict'])
        self.hidden_state = None
        
        # 创建或连接到数据库
        self.conn = sqlite3.connect('/aiarena/gpfs/code/code/OVMM/home-robot/datadump/database/train.db')
    ...
    @torch.no_grad()
    def action_predict(self,local_map,sensor_pose,target_pos,goal_iou,dd_map):
        agent_state = sensor_pose[0:3]
        agent_state = [target_pos[0] - agent_state[0], target_pos[1] - agent_state[1], agent_state[2]]
        
        current_pos = local_map[2]
        start = np.where(current_pos == 1)
        start = [int(np.mean(start[1])), int(np.mean(start[0]))]
        
        start_o = sensor_pose[2]
        arrow_length = 15
        end_x = int(start[0] + arrow_length * np.cos(np.deg2rad(start_o)))
        end_y = int(start[1] + arrow_length * np.sin(np.deg2rad(start_o)))
        end = [end_x, end_y]
        cv2.line(current_pos, start, end, 1, thickness=1)
        
        dd_map = dd_map[1:-1,1:-1]
        x_start = max(start[0] - 60, 0)
        x_end = min(start[0] + 60, dd_map.shape[1])
        y_start = max(start[1] - 60, 0)
        y_end = min(start[1] + 60, dd_map.shape[0])
        max_num_list = sorted(np.unique(dd_map[y_start:y_end,x_start:x_end]))
        sec_num,max_num = max_num_list[-2:]
        dd_map = dd_map/(1.2* sec_num)
        dd_map[dd_map > 1] = 1
        
        local_map_out = np.stack([dd_map,current_pos],axis=0)
        
        local_map_tensor = torch.tensor(local_map_out).float().to(self.device).unsqueeze(0)
        agent_state = torch.tensor(agent_state).float().to(self.device).unsqueeze(0)
        goal_iou = torch.tensor(goal_iou).float().to(self.device).unsqueeze(0)
        
        action_logits,self.hidden_state = self.action_net(
            local_map_tensor, agent_state,
            goal_iou.unsqueeze(1),self.hidden_state)
    ...
    def fetch_action_from_db(self, timestep_now):
        self.cursor.execute('SELECT * FROM action ORDER BY timestep DESC LIMIT 1')
        row = self.cursor.fetchone()

        if row is not None:
            timestep, action_blob = row
            print("timestep:",timestep)
            if timestep == timestep_now:
                # Deserialize data
                action = pickle.loads(action_blob)
                # Return the parsed data
                return {
                    "timestep": timestep,
                    "action": action
                }
        # If no new timestep or no data, return None
        return None
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        # 删除数据库中的现有数据（保持表中只有一行）
        self.cursor.execute('DELETE FROM observation')
        local_map_blob = pickle.dumps(self.semantic_map.local_map[0].cpu().numpy())
        goal_map_blob = pickle.dumps(closest_goal_map)
        rgb_blob = pickle.dumps(self.rgb_keep)
        depth_blob = pickle.dumps(self.depth_keep)
        sensor_pose_blob = pickle.dumps(planner_inputs[0]["sensor_pose"])
        dis_target_blob = pickle.dumps(dis_target)
        target_pos_blob = pickle.dumps(target_pos)
        goal_iou_blob = pickle.dumps(goal_iou)
        dd_map_blob = pickle.dumps(self.planner.dd)
        # 插入新数据到数据库
        self.cursor.execute('''
        ...
        ''', (
            self.timesteps[0],
            obs.task_observations["goal_name"],
            local_map_blob,
            goal_map_blob,
            rgb_blob,
            depth_blob,
        ...
        if save_flag:
            while True:
                time.sleep(0.1)
                action_data = self.fetch_action_from_db(self.timesteps[0])
