...
from home_robot.navigation_planner.discrete_planner_play_debug import DiscretePlanner
...
class ObjectNavAgent(Agent):
    ...
    #     dis_target = None
    #     # # print("sensor pos:",planner_inputs[0]['sensor_pose'])
    #     # now_sim = obs.now_sim
    ...
    #     # agent_start_pos = self.sim_pos2gps_pos(obs,agent_pos)
        
    #     # print("object_target_pos:",object_target_pos)
    #     # print("agent_start_pos:",agent_start_pos)
    ...
    #     return None, info, None, dis_target, self.semantic_map
    ...
    #     agent_start_pos = self.sim_pos2gps_pos(obs,agent_pos)
    #     view_points_list = [self.sim_pos2gps_pos(obs,point_tmp.agent_state.position) - agent_start_pos for point_tmp in candidate_objects.view_points]
    ...
    #     # relative_stg_list = [relative_stg] + view_points_list
    #     relative_stg_list = [relative_stg]
    ...
    #             debug=self.verbose,
    #             relative_stg = relative_stg_list,
    ...
    #         vis_inputs[0]["instance_memory"] = self.instance_memory
    #     info = {**planner_inputs[0], **vis_inputs[0]}
    #     return action, info, terminate, dis_target, self.semantic_map
    ...
    #         vis_inputs[0]["instance_memory"] = self.instance_memory
    #     info = {**planner_inputs[0], **vis_inputs[0]}
    #     return action, info, terminate, None, self.semantic_map
    
    # # exp object nav switch 
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        """Act end-to-end."""
        # t0 = time.time()
        ...
        # plt.imshow(self.semantic_map_visted_count.cpu().numpy())
        # plt.show()
    ...
    #     kernel_size = 19
    #     repeat_map = F.conv2d(
    #         self.semantic_map.global_map[:,3:4],  # 输入：轨迹图
    #         torch.ones((1, 1, kernel_size, kernel_size)).to(device=self.device),  # 卷积核：当前位置
    #         padding=kernel_size // 2  # 保持图像大小一致
    #     )

    #     near_other_path = torch.sum(repeat_map[0,0]*self.semantic_map.global_map[0,2])/(25*19*19)   # 25*19*19 max
    #     if near_other_path > 0.5:
    #         self.rl2rule_flag = True
    #         # 路径重复
    ...
    #         vis_inputs[0]["instance_memory"] = self.instance_memory
    #     info = {**planner_inputs[0], **vis_inputs[0]}
    #     return action, info, terminate, self.rl2rule_flag, self.semantic_map
