...
class ObjectNavAgent(Agent):
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        cv2.imshow("local_map[6]",local_map[6]*255)
        cv2.imshow("local_map[7]",local_map[7]*255)
        cv2.imshow("local_map[0]",local_map[0]*255)
        cv2.imshow("local_map[1]",local_map[1]*255)
        # cv2.imshow("rgb",batch_ori['robot_head_rgb'][0].cpu().numpy()[:,:,::-1])
        # cv2.imshow("obj_seg",batch_ori['ovmm_nav_goal_segmentation'][0,:,:,0].cpu().numpy().astype(np.uint8)*255)
        # recep_show = (batch['receptacle_segmentation'][0,:,:,0] == batch['start_receptacle'][0,0]).cpu().numpy().astype(np.uint8)*255
        # cv2.imshow("recep_show",recep_show)
        cv2.waitKey(1)
        ...
        elif dis_target < 2 and planner_inputs[0]["found_goal"] and np.sum((obs.semantic==1))>0:    # 只需要找到目标就行，且靠的足够近就行
            action = DiscreteNavigationAction.STOP
        else:
            (
                action,
        ...
        # save_data = {
        #     "local_map": self.semantic_map.local_map[0].cpu().numpy().astype(np.float32),  # NumPy 数组
        #     "action": action,                  # 枚举值或字符串
        #     "task_observations": obs.task_observations,  # dict
        #     "rgb": obs.rgb,
        #     "depth": obs.depth,
        ...
        # # 保存到本地文件
        # save_name = os.path.join(self.planner.local_map_dir, f"record_{self.timesteps[0]}.pkl.gz")
