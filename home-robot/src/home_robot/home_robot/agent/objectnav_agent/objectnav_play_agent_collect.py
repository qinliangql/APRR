...
from home_robot.navigation_planner.discrete_planner_play_collect import DiscretePlanner
...
class ObjectNavAgent(Agent):
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        terminate = False
        global_map = self.semantic_map.global_map[0].cpu().numpy()
        # show_map = global_map[0]*0
        # for i in range(21):
        #     show_map[global_map[i+4+5]>0] = i+9
        # for i in range(108):
        #     show_map[global_map[i+26+5]>0] = i+26+5

        if save_flag and global_map is not None and self.timesteps[0]%10==0:
            # 使用 np.packbits 将 0/1 数据压缩为更小的二进制数组
            compressed_map = np.packbits(global_map.astype(np.uint8))

            # 将压缩后的数据保存
            save_data = {
                "compressed_global_map": compressed_map,
                "original_shape": global_map.shape,  # 记录原始形状以便加载时恢复
            }
            save_name = os.path.join(self.planner.local_map_dir, f"record_{self.timesteps[0]:04d}.pkl")
    ...
    def _preprocess_obs(self, obs: Observations):
        ...
        if self.store_all_categories_in_map:
            ...
            for i in range(semantic.shape[2]):
                # obs.semantic 探索区域为23
                if 25> i > 3:
                    semantic[:,:,i][obs.semantic==i-2] = 1
                elif i > 25:
                    semantic[:,:,i][obs.semantic_obj==i-25] = 1
