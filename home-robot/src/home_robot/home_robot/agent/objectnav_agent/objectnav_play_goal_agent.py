...
class ObjectNavAgent(Agent):
    ...
    def __init__(
        ...
        # 创建或连接到数据库
        self.conn = sqlite3.connect('/aiarena/gpfs/code/code/OVMM/home-robot/data/train_db/train.db')
    ...
    def fetch_goal_map_from_db(self, timestep_now):
        self.cursor.execute('SELECT * FROM goal_map ORDER BY timestep DESC LIMIT 1')
        row = self.cursor.fetchone()

        if row is not None:
            timestep, goal_map_blob = row
            print("timestep:",timestep, " timestep_now:",timestep_now)
            if timestep == timestep_now:
                # Deserialize data
                goal_map = pickle.loads(goal_map_blob)
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        if self.timesteps[0] < self.episode_panorama_start_steps:
            ...
            goal_map_blob = pickle.dumps(goal_map_save)
            rgb_blob = pickle.dumps(obs.rgb)
            depth_blob = pickle.dumps(obs.depth)
            sensor_pose_blob = pickle.dumps(planner_inputs[0]["sensor_pose"])
            dis_target_blob = pickle.dumps(dis_target)
            # 插入新数据到数据库
            print("insert self.timesteps[0]:",self.timesteps[0])
            self.cursor.execute('''
            ...
            while True:
                time.sleep(0.1)
                goal_map_data = self.fetch_goal_map_from_db(self.timesteps[0])
                if goal_map_data is not None:
                    cv2.imshow("goal_map_data:", goal_map_data['goal_map']*255)
                    max_index = np.unravel_index(np.argmax(goal_map_data['goal_map']), goal_map_data['goal_map'].shape)
                    
                    # local_map = self.semantic_map.local_map[0].cpu().numpy()
                    # current_pos = local_map[2]
                    # start = np.where(current_pos == 1)
                    # start = [int(np.mean(start[0])), int(np.mean(start[1]))]
                    
                    # # 将 30x30 的块上采样到 120x120
                    # block_upsampled = zoom(goal_map_data['goal_map'], (4, 4), order=0)
                    
                    # max_index_pre = np.unravel_index(np.argmax(block_upsampled), block_upsampled.shape)
                    # max_index = (max_index_pre[0]-60+start[0],max_index_pre[1]-60+start[1])
                    print("max_index pre:",max_index)
                    
                    max_index_gt = np.unravel_index(np.argmax(goal_map_save[0]), goal_map_save[0].shape)
                    print("max_index_gt GT:",max_index_gt)
                    break
            (
                action,
                closest_goal_map,
        ...
        if dis_target < 0.1:
            ...
            # elif not planner_inputs[0]["found_goal"]:
                ...
                action = DiscreteNavigationAction.TURN_RIGHT 
        # if save_flag and closest_goal_map is not None:
