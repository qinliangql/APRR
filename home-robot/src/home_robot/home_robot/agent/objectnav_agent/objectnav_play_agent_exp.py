...
from home_robot.navigation_planner.discrete_planner_play_exp import DiscretePlanner
...
class ObjectNavAgent(Agent):
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        # path = self.get_oracle_action(obs,agent_pos,agent_target_pos)
        ...
        # print("goal_iou:",np.sum(goal_iou))
        # cv2.imshow("goal_iou:",goal_iou*255)
        # local_map_here = self.semantic_map.local_map[0].cpu().numpy()
        # cv2.imshow("goal objec",local_map_here[6]*255)
        # cv2.imshow("start recep",local_map_here[7]*255)
        cv2.waitKey(1)
        ...
        if dis_target < 0.1:
            ...
            # else:
                self.turn_num += 1
                action = DiscreteNavigationAction.TURN_RIGHT 
                if self.turn_num in [10,11,20,21,30,31,40,41]:
                    action = DiscreteNavigationAction.MOVE_FORWARD
                elif self.turn_num > 50:
                    print("************** STOP BY here *****************")
                    action = DiscreteNavigationAction.STOP
        else:
            self.turn_num  = 0
        ...
        if save_flag and action is not None and closest_goal_map is not None:
            save_data = {
                "local_map": self.semantic_map.local_map[0].cpu().numpy(),  # NumPy 数组
                "goal_map": closest_goal_map,              # NumPy 数组
