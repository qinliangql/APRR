...
class ObjectNavAgentModule(nn.Module):
    def __init__(self, config, instance_memory: Optional[InstanceMemory] = None):
        ...
        self.semantic_map_module = Categorical2DSemanticMapModule(
            frame_height=config.ENVIRONMENT.frame_height,   # 640
            frame_width=config.ENVIRONMENT.frame_width, # 480
            camera_height=config.ENVIRONMENT.camera_height, # 1.31
            hfov=config.ENVIRONMENT.hfov,   # 42.0
            num_sem_categories=config.AGENT.SEMANTIC_MAP.num_sem_categories,    # 21->5
            map_size_cm=config.AGENT.SEMANTIC_MAP.map_size_cm,  # 4800
            map_resolution=config.AGENT.SEMANTIC_MAP.map_resolution,    # 5
            vision_range=config.AGENT.SEMANTIC_MAP.vision_range,    # 100
            explored_radius=config.AGENT.SEMANTIC_MAP.explored_radius,  # 150
            been_close_to_radius=config.AGENT.SEMANTIC_MAP.been_close_to_radius,    # 100
            global_downscaling=config.AGENT.SEMANTIC_MAP.global_downscaling,    # 2
            du_scale=config.AGENT.SEMANTIC_MAP.du_scale,    # 4
            cat_pred_threshold=config.AGENT.SEMANTIC_MAP.cat_pred_threshold,    # 1.0
            exp_pred_threshold=config.AGENT.SEMANTIC_MAP.exp_pred_threshold,    # 1.0
            map_pred_threshold=config.AGENT.SEMANTIC_MAP.map_pred_threshold,    # 1.0
            must_explore_close=config.AGENT.SEMANTIC_MAP.must_explore_close,    # False
            min_obs_height_cm=config.AGENT.SEMANTIC_MAP.min_obs_height_cm,  # 10
            dilate_obstacles=config.AGENT.SEMANTIC_MAP.dilate_obstacles,    # True
            dilate_size=config.AGENT.SEMANTIC_MAP.dilate_size,  # 3
            dilate_iter=config.AGENT.SEMANTIC_MAP.dilate_iter,  # 1
    ...
    def forward(
        ...
        # goal_map_view = goal_map[0].cpu().numpy().astype(np.uint8)
        # cv2.imshow("goal_map_view",goal_map_view*255)
        # cv2.waitKey(1)
        ...
        # frontier_map_view = frontier_map[0,0].cpu().numpy().astype(np.uint8)
        # cv2.imshow("frontier_map_view",frontier_map_view*255)
...
class ObjectNavAgentModule2(nn.Module):
    def __init__(self, config, instance_memory: Optional[InstanceMemory] = None):
        super().__init__()
        self.instance_memory = instance_memory
        self.semantic_map_module = Categorical2DSemanticMapModule(
            frame_height=640,   # 640
            frame_width=480, # 480
            camera_height=1.31, # 1.31
            hfov=42,   # 42.0
            num_sem_categories=5,    # 21->5
            map_size_cm=4800,  # 4800
            map_resolution=5,    # 5
            vision_range=100,    # 100
            explored_radius=150,  # 150
            been_close_to_radius=100,    # 100
            global_downscaling=2,    # 2
            du_scale=4,    # 4
            cat_pred_threshold=1.0,    # 1.0
            exp_pred_threshold=1.0,    # 1.0
            map_pred_threshold=1.0,    # 1.0
            must_explore_close=False,    # False
            min_obs_height_cm=10,  # 10
            dilate_obstacles=True,    # True
            dilate_size=3,  # 3
            dilate_iter=1,  # 1
    ...
    def forward(
        ...
        # goal_map_view = goal_map[0].cpu().numpy().astype(np.uint8)
        # cv2.imshow("goal_map_view2",goal_map_view*255)
        # cv2.waitKey(1)
        seq_goal_map = goal_map.view(batch_size, sequence_length, *goal_map.shape[-2:])
        seq_found_goal = found_goal.view(batch_size, sequence_length)
