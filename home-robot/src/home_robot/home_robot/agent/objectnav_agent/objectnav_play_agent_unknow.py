...
class ObjectNavAgent(Agent):
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        if save_flag and goal_map_save is not None and action is not None and dd_map is not None:
            pre_flag = (random.random() < 0.9)
            if pre_flag:
                action_data = self.action_predict(self.semantic_map.local_map[0].cpu().numpy(),planner_inputs[0]["sensor_pose"],
                                                planner_inputs[0]["sensor_pose"][0:2]+relative_stg,
                                                (obs.semantic==1).astype(np.uint8),self.planner.dd)
                if action_data == 0:
                    action_pre = DiscreteNavigationAction.STOP 
                elif action_data == 1:
                    action_pre = DiscreteNavigationAction.MOVE_FORWARD 
                elif action_data == 2:
                    action_pre = DiscreteNavigationAction.TURN_LEFT 
                elif action_data == 3:
                    action_pre = DiscreteNavigationAction.TURN_RIGHT 
                elif action_data == 4:
                    action_pre = DiscreteNavigationAction.MOVE_FORWARD_SMALL 
        ...
        #     obs_angle = np.degrees(obs.compass[0])
        #     angle_agent = pu.normalize_angle(obs_angle)

        #     # If we found a short term goal worth moving towards...
        #     angle_st_goal = math.degrees(math.atan2(relative_stg[0], relative_stg[1]))
        #     relative_angle_to_stg = pu.normalize_angle(angle_agent - angle_st_goal)
        #     print("np.linalg.norm(relative_stg):",np.linalg.norm(relative_stg))
        #     if planner_inputs[0]["found_goal"] and np.linalg.norm(relative_stg) < 0.5:
        #         action = DiscreteNavigationAction.STOP
        #     elif relative_angle_to_stg > 30 / 2.0:
        #         action = DiscreteNavigationAction.TURN_RIGHT
        #     elif relative_angle_to_stg < -30 / 2.0:
