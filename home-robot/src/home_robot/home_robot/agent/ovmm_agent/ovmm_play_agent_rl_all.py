...
from home_robot.agent.objectnav_agent.objectnav_play_agent_rl import ObjectNavAgent
...
class PlayOpenVocabManipAgent(ObjectNavAgent):
    ...
    def _heuristic_nav(
        ...
        action, planner_info, terminate, self.dis_target, semantic_map = super().act(obs)
        # action, planner_info = super().act(obs)
        # terminate = False # 好像确实没什么用
        ...
        if action == DiscreteNavigationAction.STOP or terminate:
            ...
            terminate = False
        return action, info, terminate, semantic_map
    def _heuristic_pick(
        self, obs: Observations, info: Dict[str, Any]
    ) -> Tuple[DiscreteNavigationAction, Any]:
    ...
    def _hardcoded_place(self):
        ...
        if place_step < forward_steps:
            ...
            print("desnap to drop the object:")
    ...
    def _nav_to_obj(
        ...
        if self.skip_skills.nav_to_obj:
            ...
            action , info_a, terminate, semantic_map = self._heuristic_nav(obs, info)
            # if self.dis_target < 3:
            # if info_a['found_goal']:
            #     action, _, terminate = self.nav_to_obj_agent.act(obs, info, semantic_map, self.dis_target)  # exp + agent2
            ...
            # if (self.dis_target) and (not terminate) and (not info_a['found_goal']):
            #     pass
            # else:
            #     action = action_rl
                
            # action, _, terminate = self.nav_to_obj_agent.act(obs, info, semantic_map, info_a)   # rl agent1 agent2
            
            # action, _, terminate = self.nav_to_obj_agent.act(obs, info)  # all to rl
            info = info_a
        else:
            raise ValueError(
    ...
    def _nav_to_rec(
        ...
        if self.skip_skills.nav_to_rec:
            ...
            action, info, terminate,_ = self._heuristic_nav(obs, info)
