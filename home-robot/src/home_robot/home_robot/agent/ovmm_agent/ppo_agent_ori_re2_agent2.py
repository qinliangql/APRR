...
class PPOAgent(Agent):
    ...
    def __init__(
        ...
        self.actor_critic = policy.from_config_ori_re2(
            self.rl_config,
            skill_obs_spaces,
        ...
        model_path = '/aiarena/gpfs/code/code/OVMM/home-robot_ini/home-robot/src/third_party/habitat-lab/data/new_checkpoints/ovmm/nav_to_obj_ori_re2/keep5/latest_keep.pth'
        if model_path:
        ...
        self.actor_critic2 = policy.from_config2(
            self.rl_config,
            skill_obs_spaces,
            self.vector_action_space,
        )
        ...
        if model_path2:
            ...
            #  Filter only actor_critic weights
            self.actor_critic2.load_state_dict(
                {
                    k[len("actor_critic.") :]: v
                    for k, v in ckpt["state_dict"].items()
                    if "actor_critic" in k
        ...
        print("skill_config.discrete_turn_degrees:",skill_config.discrete_turn_degrees)
    ...
    def reset(self) -> None:
        ...
        self.test_recurrent_hidden_states2 = torch.zeros(
            1,
            self.actor_critic2.num_recurrent_layers,
            self.hidden_size,
            device=self.device,
            dtype=torch.float32,
        )
    ...
    def _get_short_goal(self, obs: Observations) -> np.ndarray:
        agent_pos = obs.now_sim.robot.base_pos
        # candidate_objects = obs.episode.candidate_objects_hard[0]
        goals = mn.Vector3(obs.task.nav_goal_pos[0])
        # object_target_pos = self.sim_pos2gps_pos(obs,candidate_objects.position)
    
        # path = self.get_oracle_action(obs,agent_pos,goals)
        # print("path:",len(path),path)
        # return path[1] - path[0]
        
        agent_pos = self.sim_pos2gps_pos(obs, agent_pos)
        pos = self.sim_pos2gps_pos(obs, goals)
        
        # print("goal_pos:",pos)
        # print("agent_pos:",agent_pos)

        return pos - agent_pos
    ...
    def convert_to_habitat_obs_space(
        ...
        # hab_obs["ovmm_object_position"] = self._get_short_goal(obs)
        
        return hab_obs

    def act(
        self, observations: Observations, info, semantic_map,info_a
    ) -> Tuple[
        Union[
            ContinuousFullBodyAction,
            ContinuousNavigationAction,
        ...
        # self.local_map = F.interpolate(semantic_map.local_map[:,[0,2,3]], scale_factor=0.5, mode='bilinear', align_corners=True)
        self.local_map = F.interpolate(semantic_map.global_map[:,[2,3,6,7]], scale_factor=0.25, mode='bilinear', align_corners=True)
        ...
        # skill_obs_keys_tmp = ["joint", "ovmm_nav_goal_segmentation", "ovmm_object_position", "receptacle_segmentation", "robot_start_compass", "robot_start_gps", "robot_head_depth", "start_receptacle"]
        # skill_obs_keys_tmp = ["joint", "ovmm_nav_goal_segmentation", "receptacle_segmentation", "robot_start_compass", "robot_start_gps", "robot_head_depth", "start_receptacle"]
        # skill_obs_keys_tmp = [ "ovmm_object_position","ovmm_nav_goal_segmentation", "robot_start_compass", "robot_start_gps", "robot_head_depth"]
        # batch = OrderedDict([(k, batch[k]) for k in skill_obs_keys_tmp])
        batch = OrderedDict([(k, batch[k]) for k in self.skill_obs_keys])
        # print("batch:",batch.keys())
        batch['local_map'] = self.local_map

        if self.show_rl_obs:
            frame = observations_to_image(viz_obs, info={})
            info["rl_obs_frame"] = frame
        # show_img = batch['robot_head_depth'].cpu().numpy()[0,:,:,0] 
        # plt.imshow(show_img)
        # plt.savefig("show.png")
        # dis = (batch['ovmm_object_position'][0][0])**2 + (batch['ovmm_object_position'][0][1])**2
        
        # batch['ovmm_object_position'] = batch['ovmm_object_position'].clone()
        # relative_stg = batch['ovmm_object_position'][0]
        # sensor_pose = semantic_map.get_planner_pose_inputs(0)
        # start_x, start_y, start_o, gx1, gx2, gy1, gy2 = sensor_pose
        # gx1, gx2, gy1, gy2 = int(gx1), int(gx2), int(gy1), int(gy2)
        # set_goal = [
        #     int((start_y + relative_stg[0])*100.0/5 - gx1),
        #     int((start_x + relative_stg[1])*100.0/5 - gy1),
        # ]
        # set_goal = pu.threshold_poses(set_goal, semantic_map.local_map[0,0].shape)
        
        # start = [
        # int(start_y * 100.0 / 5 - gx1),
        # int(start_x * 100.0 / 5 - gy1),
        # ]
        # start = pu.threshold_poses(start, semantic_map.local_map[0,0].shape)
        
        # batch['ovmm_object_position'][0] = torch.from_numpy(np.array(set_goal) - np.array(start))
        ...
        with torch.no_grad():
            ...
            #     self.test_recurrent_hidden_states2,
            # ) = action_data
            if (not info_a['found_goal']) and (not self.change_flag):
            ...
            if self.continuous_actions: # False
