...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    img_show = "/aiarena/gpfs/code/code/OVMM/home-robot/mytest/points.jpg"
    # img_show = None
    ...
    def __init__(
        ...
        # # Initialize geoms as an empty list, will be updated later in the main thread
        # self.geoms = []
        
        # # Create a thread to visualize geoms
        # 创建或连接到数据库
        # self.conn = sqlite3.connect('/aiarena/gpfs/code/code/OVMM/home-robot/mytest/pointcloud/pointcloud_data.db')
        ...
        # # 检查表是否存在并清空数据
        # self.cursor.execute('DELETE FROM pointcloud_data')
        # self.conn.commit()
        
        self.debug_mode = False
        # Initialize an empty container for the aggregated point cloud
        # self.aggregated_point_cloud = np.empty((0, 3),dtype=np.float32)
        # self.aggregated_rgb = np.empty((0, 3),dtype=np.float32)
    ...
    def _update_local_map_and_pose(
        ...
        if camera_pose is not None:
            ...
            angles = matrix_to_euler_angles(camera_pose[:, :3, :3], convention="ZYX")
            # angles = torch.Tensor(
        ...
        if self.debug_mode:
            ...
            rgb = obs[:, :3, :: self.du_scale, :: self.du_scale].permute(0, 2, 3, 1)    # torch.Size([1, 160, 120, 3])
            # rgb = (obs[:, 5:6, :: self.du_scale, :: self.du_scale].permute(0, 2, 3, 1).repeat(1,1,1,3)+1)*100   # torch.Size([1, 160, 120, 3])
            # cv2.imshow("obs_show",obs[0,5,:,:].cpu().numpy()*255)
            # cv2.waitKey(1)
            # rgb_mask = obs[:, :3]
            # rgb_mask[obs[:, 5]>0] = [0,255,0]
        ...
        # point_cloud_base_coords = du.transform_camera_view_t(
        #     point_cloud_t, agent_height, (angles[0]).cpu().numpy(), device
        # )
        ...
        shift_global = (current_pose[0,:] + origins[0,:]).cpu().float().numpy().astype(np.float64)
        shift_global = [shift_global[0],shift_global[1],np.radians(shift_global[2])]
        point_cloud_gloabl_map_coords = du.transform_pose_t_global(
            point_cloud_base_coords, shift_global, device
        )
        # point_cloud_gloabl_map_coords = du.transform_point_cloud_direct(point_cloud_t,camera_pose,current_pose)
        ...
        if self.debug_mode:
            ...
            # xyz = point_cloud_gloabl_map_coords[0].reshape(-1, 3)
            # print("-> Showing point cloud in map coords")
            # geoms = show_point_cloud(
            #     (xyz / 100.0).cpu().numpy(),
            #     (rgb / 255.0).cpu().numpy(),
            #     orig=np.zeros(3),
            #     save = self.img_show,
            ...
            xyz = (xyz/100).cpu().numpy().astype(np.float32)
            rgb = (rgb / 255.0).cpu().numpy().astype(np.float32)
            rgb_bytes = rgb.tobytes()
            xyz_bytes = xyz.tobytes()

            # Save the shapes as strings so we can reshape the arrays later
            rgb_shape = str(rgb.shape)
            xyz_shape = str(xyz.shape)
            # Insert data into the database
            self.cursor.execute('''
                INSERT INTO pointcloud_data (rgb_data, xyz_data, rgb_shape, xyz_shape)
                VALUES (?, ?, ?, ?)
            ''', (rgb_bytes, xyz_bytes, rgb_shape, xyz_shape))
