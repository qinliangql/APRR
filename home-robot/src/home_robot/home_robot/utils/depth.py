...
# def transform_camera_view_t(XYZ, sensor_height, camera_angles, device):
#     """
#     Transforms the point云 into geocentric frame considering all rotation angles
#     Input:
#         XYZ                : ...x3
#         sensor_height      : height of the sensor
#         camera_angles      : [yaw, pitch, roll] angles in degrees
...
#     # 绕X轴旋转(俯仰角/倾斜角)
#     Rx = ru.get_r_matrix([1.0, 0.0, 0.0], angle=camera_angles[1])
    
#     # 绕Y轴旋转(翻滚角)
#     Ry = ru.get_r_matrix([0.0, 1.0, 0.0], angle=camera_angles[2])
    
#     # 应用旋转
#     # XYZ = torch.matmul(
#     #     XYZ.reshape(-1, 3), 
#     #     torch.from_numpy(Ry).float().transpose(1, 0).to(device)
#     # ).reshape(XYZ.shape)
    
#     XYZ = torch.matmul(
#         XYZ.reshape(-1, 3), 
#         torch.from_numpy(Rx).float().transpose(1, 0).to(device)
#     ).reshape(XYZ.shape)
...
def transform_pose_t_global(XYZ, current_pose, device):
    ...
    # 将点云平移回原来的坐标系
    XYZ[..., 0] += current_pose[0]*100
    XYZ[..., 1] += current_pose[1]*100
    
    return XYZ

# def transform_point_cloud_with_camera_pose(point_cloud_t, camera_pose):
#     # 将点云展平为 [B, H*W, 3]
#     point_cloud_flat = point_cloud_t.reshape(point_cloud_t.shape[0], -1, 3)
    
#     # 添加齐次坐标 [B, H*W, 4]
#     point_cloud_homogeneous = torch.cat([
#         point_cloud_flat, 
#         torch.ones_like(point_cloud_flat[..., :1])
#     ], dim=-1)
    
#     # 直接应用camera_pose变换
#     # 注意：需要转置camera_pose的变换矩阵
#     transformed_point_cloud = torch.bmm(
#         point_cloud_homogeneous, 
#         camera_pose[:, :3, :].transpose(1, 2)
#     )[..., :3]
    
#     # 恢复原始形状
#     transformed_point_cloud = transformed_point_cloud.reshape(point_cloud_t.shape)
    
#     return transformed_point_cloud

# def transform_point_cloud_direct(point_cloud_t, camera_pose, current_pose):
#     """
#     直接将相机坐标系点云转换到全局坐标系
    
#     Args:
#     point_cloud_t: 相机坐标系下的点云 [B, H, W, 3]
#     camera_pose: 相机位姿矩阵 [B, 4, 4]
    
#     Returns:
#     transformed_point_cloud: 全局坐标系下的点云
#     """
#     # 提取旋转矩阵 (前3x3子矩阵)
#     rotation_matrix = camera_pose[:, :3, :3]
    
#     # 提取平移向量
#     translation_vector = camera_pose[:, :3, 3] * 100
    
#     # 将点云展平
#     point_cloud_flat = point_cloud_t.reshape(point_cloud_t.shape[0], -1, 3)
    
#     # 先旋转
#     rotated_point_cloud = torch.matmul(point_cloud_flat, rotation_matrix.transpose(1, 2))
    
#     # 再平移
#     transformed_point_cloud = rotated_point_cloud + translation_vector.unsqueeze(1)
